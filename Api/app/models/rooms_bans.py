...
class RoomsBans:
	...
	def get(self, room_id: int) -> orm.RoomsBans:
		...
		"""
		with self._database.scope as scope:
			return scope.query(orm.RoomsBans).filter(orm.RoomsBans.id == room_id).one()
	...
	def get_all_visible(self, user_id: int, room_id_filter: int = None, banner_id_filter: int = None) -> typing.List[orm.RoomsBans]:
		with self._database.scope as scope:
			query = scope.query(orm.RoomsBans).filter(
				or_(
					# Bans of which room the user has created
					orm.RoomsBans.room_id.in_(
						scope.query(orm.Rooms.id).filter(orm.Rooms.user_id == user_id)
					),
					# Bans which user has created
