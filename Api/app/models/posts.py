...
class Posts:
	...
	def get_all(self, exclude_banned: bool, exclude_public: bool, exclude_private: bool, user_id: int = None, room_id_filter: int = None, user_id_filter: int = None):
		with self._database.scope as scope:
			q_visible_ids = []
			...
			query = scope.query(orm.Posts).filter(
				or_(
					orm.Posts.id.in_(q_visible_ids),
					orm.Posts.id.in_(q_filtered_ids)
				)
			)
			if room_id_filter is not None:
				query = query.filter(orm.Posts.room_id == room_id_filter)
			if user_id_filter is not None:
				query = query.filter(orm.Posts.user_id == user_id)
			return query.all()
